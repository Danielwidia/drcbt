...
const sqlDb = require('./db'); // ← MySQL database module
const mysqlMgr = require('./mysql-manager'); // ← Portable MySQL Manager
...
// ─── Environment ──────────────────────────────────────────────────────────────
// Mode: MySQL via environment variables sebagai database utama.
// Supabase hanya digunakan untuk sinkronisasi manual via admin dashboard.
const USE_SUPABASE = false; // Local mode - gunakan tombol Sync di Admin untuk sinkronisasi Supabase
let supabase = null;        // Supabase client (dibuat saat dibutuhkan untuk sync)
...
console.log(`💾 Database Mode : MySQL (backend)`);
console.log(`   Connection: ${sqlDb.getDbPath()}`);

// Jika database MySQL masih kosong atau belum berisi data awal
async function autoMigrateIfNeeded() {
    ...
    try {
        ...
        console.log('[INIT] Checking migration status via MySQL...');
        let hasMigrated = false;
        try {
            hasMigrated = sqlDb.getConfig('migrated_from_sqlite');
        } catch (e) {
            console.warn('[INIT] Could not check config table (might be first run):', e.message);
        }

        if (hasMigrated === true) {
            console.log('[INIT] Database already migrated from SQLite previously. Skipping auto-migration.');
            return;
        }
        console.log('[INIT] Migration flag is not set.');

        console.log('[INIT] Checking existing students via MySQL...');
        let existing = [];
        try {
            existing = sqlDb.getAllStudents();
            ...
            // Don't proceed with migration if DB is malformed/inaccessible
            return;
        }

        if (existing.length === 0) {
            console.log('[INIT] Database tables are empty. Checking source files for migration...');
            console.log('[INIT] Students table is empty. Starting migration check...');
...
async function readDB(loadAll = true) {
    ...
    // ← MySQL mode
    try {
        const dbObj = sqlDb.readDB(loadAll);
        return dbObj || { ...DEFAULT_DB, globalSettings: { apiKeys: [] } };
    } catch (e) {
        console.error('readDB (MySQL) error:', e.message);
        // Kembali ke default agar server tidak crash total
        return {
            ...DEFAULT_DB,
...
async function readResults(options = {}) {
    if (USE_SUPABASE) {
        ...
        const { data, error } = await query
            ...
            .limit(options.limit || RESULT_FETCH_LIMIT)
            .range(options.offset || 0, (options.offset || 0) + (options.limit || RESULT_FETCH_LIMIT) - 1);
    ...
    try {
        if (Object.keys(options).length > 0) {
            return sqlDb.getResults(options);
        }
        return sqlDb.getAllResults();
    } catch (e) {
        console.error('readResults (MySQL) error:', e.message);
        return [];
    }
}
...
async function readLiveExams() {
    if (USE_SUPABASE) {
        const { data, error } = await supabase
            ...
            .select('data');
        if (error) {
            const err = new Error(error.message || 'Supabase readLiveExams error');
            err.code = error.code;
            throw err;
        }
        console.log('[readLiveExams][Supabase] Fetched', data.length, 'records');
        return data.map(row => row.data);
    }
    // ← MySQL mode
    try {
        const items = sqlDb.getAllLiveExams();
        console.log('[readLiveExams][MySQL] Parsed', items.length, 'items');
        return items;
    } catch (e) {
        console.error('[readLiveExams][MySQL] Error:', e.message);
        return [];
    }
}
    ...
    if (USE_SUPABASE) {
        for (const exam of liveExams) {
            ...
            const { data: existing, error: selectError } = await supabase
                ...
                .maybeSingle();
            if (selectError) console.error('Supabase select live exam error:', selectError.message);
            if (existing) {
                const { error: updateError } = await supabase.from('cbt_live_exams').update(record).eq('id', existing.id);
                if (updateError) console.error('Supabase update live exam error:', updateError.message);
            } else {
                const { error: insertError } = await supabase.from('cbt_live_exams').insert(record);
                if (insertError) console.error('Supabase insert live exam error:', insertError.message);
            }
        }
        return;
    }
    // ← MySQL mode
    sqlDb.setAllLiveExams(liveExams);
}
...
async function insertLiveExamSingle(exam) {
    ...
    if (USE_SUPABASE) {
        ...
        console.log('[insertLiveExamSingle] Saved to Supabase');
        return;
    }
    // ← MySQL mode
    try {
        ...
        const existing = sqlDb.getAllLiveExams().find(e =>
            norm(e.studentId) === nid &&
            norm(e.rombel) === nrb &&
            norm(e.mapel) === nmp
        );

        // HARD WIPE: If this is a deletion request, use a direct DB update first to be 100% sure
        if (exam.adminDeleteCheckpoint) {
            console.log(`[insertLiveExamSingle] 🚀 HARD WIPING database checkpoint for ${nid}`);
            sqlDb.clearCheckpointDirectly(nid, nmp, nrb);

            // Also ensure the current 'existing' and 'merged' objects are cleaned
            if (existing) {
        ...
        // Final safeguard for merged result
        if (exam.adminDeleteCheckpoint) {
            merged.adminSavedProgress = null;
            merged.adminSaveConfirmed = false;
            merged.savedByAdminCommand = false;
            merged.adminClearRequest = Date.now(); // Ensure student gets the versioned command
        }

        sqlDb.upsertLiveExam(merged);
        console.log('[insertLiveExamSingle] Saved to MySQL (normalized)');
    } catch (err) {
        console.error('[insertLiveExamSingle] MySQL Error:', err.message);
        throw err;
    }
}
